...
namespace sdds
{
  ...
  CustomerOrder::CustomerOrder(const std::string& src)
  {
    ...
    try {
      m_name = util.extractToken(src, pos, more); //here
      ...
      while (more && m_cntItem) {
          //m_lstItem[--idx] = new Item(util.extractToken(src, pos, more)); //problem here
        //m_lstItem[idx + 1] = new Item(util.extractToken(src, pos, more)); //problem here
        m_lstItem[idx] = new Item(util.extractToken(src, pos, more)); //problem here
        ++idx;
      }
